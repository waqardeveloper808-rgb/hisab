// Workspace V2 — schema-driven PDF export (Wafeq Format 2 coordinates).
//
...
import { containsArabic, shapeArabicForPdf } from "@/lib/workspace-v2/arabic-pdf-text";
...
import {
  ...
  type LangMode,
  type SectionKey,
} from "@/lib/workspace-v2/document-template-schemas";
import {
  buildDocumentLayout,
  type LayoutInfoRow,
  ...
  type RenderCustomer,
  type RenderSeller,
} from "@/lib/workspace-v2/document-template-renderer";
...
function drawHeader(ctx: DrawCtx, layout: LayoutPlan, language: LangMode, sec: LayoutSection, yPx: number): number {
  ...
  let ty = rowY + enPad;
  if (showEn) {
    drawText(ctx, layout.seller.nameEn, px(enInnerL), topY(ty) - 13, 13, ctx.helvBold, enBody);
    ty += 18;
    if (layout.seller.addressEn) {
      const wrapped = wrapByWidth(layout.seller.addressEn, ctx.helv, TYPOGRAPHY.bodyPx - 1, px(enInnerW), ctx.warnings);
      for (const wv of wrapped.slice(0, 4)) {
        drawText(ctx, wv, px(enInnerL), topY(ty) - 9, 9, ctx.helv, enBody);
        ty += 11;
      }
    }
    if (layout.seller.email) {
      drawText(ctx, layout.seller.email, px(enInnerL), topY(ty) - 9, 9, ctx.helv, enBody);
      ty += 11;
    }
    if (layout.seller.vatValue) {
      const line = `${layout.seller.vatLabelEn} ${layout.seller.vatValue}`;
      drawText(ctx, line, px(enInnerL), topY(ty) - 9, 9, ctx.helv, enBody);
      ty += 11;
    }
  ...
  if (showAr && (layout.seller.nameAr || layout.seller.nameEn)) {
    ...
    const shaped = shapeArabicForPdf(nameAr);
    const fs = 13;
    const nw = ctx.arFont.widthOfTextAtSize(shaped, fs);
  ...
  if (showAr && layout.seller.addressAr) {
    ...
    for (const wv of wrapped.slice(0, 4)) {
      ...
      const fs = 9;
      const nw = ctx.arFont.widthOfTextAtSize(sh, fs);
  ...
  if (showAr && layout.seller.email) {
    ...
    const fs = 9;
    const w = ctx.helv.widthOfTextAtSize(t, fs);
    drawText(ctx, t, px(arInnerR) - w, topY(ry) - 9, fs, ctx.helv, arBody);
    ry += 11;
  }
  if (showAr && layout.seller.vatValue) {
    const line = `${layout.seller.vatLabelAr} ${layout.seller.vatValue}`;
    const f = fontForString(ctx, line, ctx.helv);
    const pl = f === ctx.arFont ? shapeArabicForPdf(line) : line;
    const fs = 9;
    const w = f.widthOfTextAtSize(pl, fs);
    ctx.page.drawText(pl, { x: px(arInnerR) - w, y: topY(ry) - fs, size: fs, font: f, color: arBody });
    ry += 11;
  }
  if (showAr && layout.seller.crValue) {
    const line = `${layout.seller.crLabelAr} ${layout.seller.crValue}`;
    const f = fontForString(ctx, line, ctx.helv);
    const pl = f === ctx.arFont ? shapeArabicForPdf(line) : line;
    const fs = 9;
    const w = f.widthOfTextAtSize(pl, fs);
...
function drawInfoTable(
  ...
  for (const row of rows) {
    ...
    drawTextAlign(ctx, leftLabel, il.englishAlign, xEn, enW, cursorY + 4, 10, ctx.helv, enRgb, 0);
    drawTextAlign(ctx, row.value || "—", il.valueAlign, xVal, valW, cursorY + 4, 11, ctx.helv, enRgb, 0);
...
function drawItems(ctx: DrawCtx, layout: LayoutPlan, language: LangMode, sec: LayoutSection, yPx: number): number {
  const topPad = 8;
  const enHeaderH = language === "arabic" ? 0 : 18;
  const arHeaderH = language === "english" ? 0 : 16;
  ...
  if (language !== "arabic") {
    ...
    for (const col of scaledCols) {
      drawTextAlign(ctx, col.labelEn, col.align, cl, col.widthPx, headerY + 2, 9, ctx.helvBold, enRgb);
      cl += col.widthPx;
    }
  }
  if (language !== "english") {
    ...
    let cl = tableX;
    for (const col of scaledCols) {
      drawArCellHeader(ctx, col.labelAr, col, cl, arY + 2, 8, arRgb);
      cl += col.widthPx;
    }
  }
...
function drawTotalsBlock(ctx: DrawCtx, layout: LayoutPlan, language: LangMode, sec: LayoutSection, yPx: number): number {
  ...
  for (const row of layout.totalsRows) {
    const labelSize = row.emphasis ? 12 : 11;
    const valueSize = row.emphasis ? 13 : 11;
...
function drawQrBlock(ctx: DrawCtx, layout: LayoutPlan, language: LangMode, sec: LayoutSection, yPx: number): number {
  ...
  if (language !== "arabic") {
    ...
    for (const wv of wrapped.slice(0, 4)) {
      const wpt = ctx.helv.widthOfTextAtSize(wv, 9);
      let xPt: number;
      if (qb.align === "left") xPt = captionXLeftPt;
      else if (qb.align === "right") xPt = captionXRightPt - wpt;
      else xPt = captionXCenterPt - wpt / 2;
      ctx.page.drawText(wv, { x: xPt, y: topY(advY(ly, ctx)) - 9, size: 9, font: ctx.helv, color: subtleColor });
      ly += 11;
    }
  }
  if (language !== "english") {
    const wrapped = wrapByWidth(layout.qr.captionAr, ctx.arFont, 9, px(captionW), ctx.warnings, true);
    for (const wv of wrapped.slice(0, 4)) {
      const sh = shapeArabicForPdf(wv);
      const wpt = ctx.arFont.widthOfTextAtSize(sh, 9);
      let xPt: number;
      if (qb.align === "left") xPt = captionXLeftPt;
      else if (qb.align === "right") xPt = captionXRightPt - wpt;
      else xPt = captionXCenterPt - wpt / 2;
      ctx.page.drawText(sh, { x: xPt, y: topY(advY(ly, ctx)) - 9, size: 9, font: ctx.arFont, color: subtleColor });
      ly += 12;
    }
  }
...
function drawStampSignature(ctx: DrawCtx, layout: LayoutPlan, language: LangMode, sec: LayoutSection, yPx: number): number {
  ...
  for (const b of blocks) {
    ...
    if (isSignature && ctx.signatoryName) {
      drawTextAlign(ctx, ctx.signatoryName, "center", b.x, b.w, lineY - 18, 9, ctx.helv, rgbFromLayoutEn(layout), 0);
    }
    ...
    if (language === "english") {
      drawTextAlign(ctx, b.labelEn, "center", b.x, b.w, labelY, 9, ctx.helv, capRgb, 0);
    } else if (language === "arabic") {
      const sh = shapeArabicForPdf(b.labelAr);
      drawTextAlign(ctx, sh, "center", b.x, b.w, labelY, 9, ctx.arFont, rgbFromLayoutAr(layout), 0);
    } else {
      const pre = `${b.labelEn} / `;
      const wP = ctx.helv.widthOfTextAtSize(pre, 9);
      const shA = shapeArabicForPdf(b.labelAr);
      const wA = ctx.arFont.widthOfTextAtSize(shA, 9);
      const totW = wP + wA;
      const mid = (px(advX(b.x, ctx)) + px(advX(b.x + b.w, ctx))) / 2;
      const yDraw = topY(advY(labelY, ctx)) - 9;
      const x0 = mid - totW / 2;
      ctx.page.drawText(pre, { x: x0, y: yDraw, size: 9, font: ctx.helv, color: capRgb });
      ctx.page.drawText(shA, { x: x0 + wP, y: yDraw, size: 9, font: ctx.arFont, color: rgbFromLayoutAr(layout) });
    }
  }
  return h;
}
