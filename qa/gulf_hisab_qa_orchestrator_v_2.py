...
class CTAClarityChecker(BaseChecker, HTMLFetchMixin):
    ...
    def run(self, target: Dict[str, Any], context: Dict[str, Any]) -> CheckResult:
        ...
        for node in buttons:
            ...
            lowered = text.lower()
            if (node.name == "button" or href) and not text:
                empty_actions.append({"tag": node.name, "href": href, "class": classes})
            elif text and (len(text) < 3 or lowered in banned_cta_words):
                vague_labels.append({"text": text, "tag": node.name, "href": href, "class": classes})
...
class RouteAvailabilityChecker(BaseChecker, HTMLFetchMixin):
    ...
    def run(self, target: Dict[str, Any], context: Dict[str, Any]) -> CheckResult:
        ...
        reject_text = [item.lower() for item in target.get("reject_text", ["page not found", "404"])]
        ...
        if response_status_issue:
            ...
            response_status_issue.description = "This route did not return a successful response."
...
class LogoPresenceChecker(BaseChecker, HTMLFetchMixin):
    ...
    def run(self, target: Dict[str, Any], context: Dict[str, Any]) -> CheckResult:
        ...
        passed = not any(i.severity in {"critical", "high"} for i in issues)
        return CheckResult(self.name, url, passed, max(score, 0), "Checked for official logo presence.", issues, {"logo_count": len(found)})
...
class ArchitectureConventionChecker(BaseChecker):
    ...
    def run(self, target: Dict[str, Any], context: Dict[str, Any]) -> CheckResult:
        ...
        for pattern in context.get("anti_patterns", []):
            risky_files.extend([str(p) for p in repo_root.rglob(pattern)])
        if risky_files:
...
def default_config() -> Dict[str, Any]:
    return {
        ...
        "banned_ui_labels": ["module", "tag", "badge", "feature card", "structured placeholder", "sandbox mode"],
