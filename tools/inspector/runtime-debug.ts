...
async function main() {
  ...
  const registerRowButton = page.locator("table[data-inspector-row-clickable='true'] tbody tr button[data-inspector-row-clickable='true']").first();
  await registerRowButton.waitFor({ state: "visible", timeout: 30000 });

  const initialRows = await page.locator("table[data-inspector-row-clickable='true'] tbody tr").count();
  const layoutMetrics = await page.evaluate(() => {
    const documentWidth = document.documentElement.scrollWidth;
    const viewportWidth = window.innerWidth;
    const table = document.querySelector("table[data-inspector-row-clickable='true']");
    ...
    let scrollParent: HTMLElement | null = table?.parentElement ?? null;
