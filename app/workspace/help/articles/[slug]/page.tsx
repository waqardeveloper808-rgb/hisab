      ...
      <div className="grid gap-5 lg:grid-cols-[minmax(0,1fr)_18rem]">
        <div className="space-y-5">
          {article.sections.map((section) => (
            <Card key={section.heading} className="rounded-[1.8rem] bg-white/92 p-6">
        ...
        <Card className="rounded-[1.8rem] bg-white/92 p-6 lg:sticky lg:top-28 lg:h-fit">
          ...
          <div className="mt-4 space-y-3 text-sm leading-6 text-muted">
            {helpArticles.filter((entry) => entry.slug !== article.slug).map((entry) => (
              <Link key={entry.slug} href={`/workspace/help/articles/${entry.slug}`} className="block rounded-[1.2rem] border border-line bg-surface-soft px-4 py-3 hover:border-primary/25 hover:bg-white">
