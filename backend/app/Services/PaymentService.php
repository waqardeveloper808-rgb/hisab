...
class PaymentService
{
    ...
    public function recordIncomingPayment(Company $company, Document $document, User $user, array $payload): Payment
    {
        return $this->recordIncomingPaymentForAllocations($company, $user, [
            ...
            'discount_allowed_amount' => $payload['discount_allowed_amount'] ?? 0,
            'allocations' => [[
    ...
    public function recordIncomingPaymentForAllocations(Company $company, User $user, array $payload): Payment
    {
        ...
        $discountAllowed = BigDecimal::of((string) ($payload['discount_allowed_amount'] ?? 0))->toScale(2);
        $allocationDiscounts = collect($payload['allocation_discounts'] ?? []);
        ...
        if ($discountAllowed->isLessThan(BigDecimal::zero())) {
            throw ValidationException::withMessages([
                'discount_allowed_amount' => 'Discount allowed cannot be negative.',
            ]);
        }

        $allocations = collect($payload['allocations'] ?? [])
            ->map(fn (array $allocation) => [
                'document_id' => isset($allocation['document_id']) ? (int) $allocation['document_id'] : null,
                'amount' => BigDecimal::of((string) $allocation['amount'])->toScale(2),
            ]);

        return DB::transaction(function () use ($company, $user, $payload, $amount, $discountAllowed, $allocationDiscounts, $allocations): Payment {
            ...
            if ($allocationDiscounts->isEmpty() && $allocations->count() === 1 && $discountAllowed->isGreaterThan(BigDecimal::zero())) {
                $allocationDiscounts = collect([(string) $allocations->first()['document_id'] => (string) $discountAllowed]);
            }

            foreach ($allocations as $index => $allocation) {
                if (! $allocation['document_id']) {
                    throw ValidationException::withMessages([
                        "allocations.$index.document_id" => 'Each allocation must be linked to an invoice.',
                    ]);
                }
                ...
                $documentBalance = BigDecimal::of((string) $document->balance_due);
                $lineDiscount = BigDecimal::of((string) ($allocationDiscounts->get((string) $allocation['document_id'], 0) ?: 0))->toScale(2);

                if ($lineDiscount->isLessThan(BigDecimal::zero())) {
                    throw ValidationException::withMessages([
                        "allocations.$index.discount_allowed_amount" => 'Allocation discount cannot be negative.',
                    ]);
                }

                if ($allocation['amount']->plus($lineDiscount)->isGreaterThan($documentBalance)) {
                    throw ValidationException::withMessages([
                        "allocations.$index.amount" => 'Allocation amount plus discount cannot exceed the document balance due.',
                    ]);
                }
            ...
            if ($allocatedTotal->plus($discountAllowed)->isGreaterThan($amount->plus($discountAllowed))) {
                throw ValidationException::withMessages([
                    'allocations' => 'Allocated amounts and discounts are inconsistent with the payment total.',
                ]);
            }
            ...
            foreach ($allocations as $allocation) {
                ...
                $document->update([
                    ...
                    'custom_fields' => array_merge($document->custom_fields ?? [], [
                        'discount_allowed_amount' => (string) BigDecimal::of((string) (($document->custom_fields['discount_allowed_amount'] ?? '0')))->plus($allocationDiscount)->toScale(2),
                    ]),
            ...
            $this->ledgerService->postIncomingPayment(
                ...
                $allocations->map(fn (array $allocation) => [
                    'document_id' => $allocation['document_id'],
                    'amount' => (string) $allocation['amount'],
                    'discount_allowed_amount' => (string) BigDecimal::of((string) $allocationDiscounts->get((string) $allocation['document_id'], '0'))->toScale(2),
                ])->values()->all(),
            );
            ...
            foreach ($documents as $document) {
                ...
                $this->accountingIntegrityService->validateIncomingPayment(
                    $company,
                    $payment->fresh('allocations'),
                    $document->fresh(),
                    $user->id,
                );
            }
