...
class SalesDocumentController extends Controller
{
    ...
    public function store(Request $request, Company $company): JsonResponse
    {
        ...
        if (in_array($payload['type'], ['tax_invoice', 'cash_invoice', 'api_invoice'], true)) {
            $this->planLimitService->ensureInvoicingTrialWindow($company);
    ...
    public function issueDebitNote(Request $request, Company $company, Document $document): JsonResponse
    {
        $this->ensureCompanyAbility($request->user(), $company, 'workspace.sales.manage');
        $this->ensureDocumentOwnership($company, $document);
        ...
        $debitNote = $this->salesDocumentService->issueDebitNote($company, $document, $request->user(), $payload);

        return response()->json(['data' => $debitNote], 201);
    }
    ...
    private function validatePayload(Request $request, bool $includeType = true): array
    {
        $rules = [
            ...
            'reversal_of_document_id' => ['nullable', 'integer'],
            'status_reason' => ['nullable', 'string', 'max:255'],
        ...
        if ($includeType) {
            $rules['type'] = ['required', 'in:quotation,proforma_invoice,delivery_note,tax_invoice,recurring_invoice,cash_invoice,api_invoice,debit_note'];
        }
