...
class DocumentCenterController extends Controller
{
    ...
    public function index(Request $request, Company $company): JsonResponse
    {
        ...
        if (($payload['group'] ?? 'all') === 'sales') {
            $query->whereIn('type', ['quotation', 'proforma_invoice', 'delivery_note', 'tax_invoice', 'credit_note', 'debit_note', 'recurring_invoice', 'cash_invoice', 'api_invoice']);
        }

        if (($payload['group'] ?? 'all') === 'purchase') {
            $query->whereIn('type', ['vendor_bill', 'purchase_invoice', 'purchase_credit_note', 'purchase_order']);
        }
    ...
    public function exportPdf(Request $request, Company $company, Document $document)
    {
        $this->ensureCompanyAccess($request->user(), $company);
        $this->ensureDocumentOwnership($company, $document);
        $this->planLimitService->ensureAccountingPrintAccess($company, $document);
        ...
        return Pdf::loadHTML($this->renderer->renderHtml($company, $document, $templateOverride))
    ...
    public function send(Request $request, Company $company, Document $document): JsonResponse
    {
        ...
        $this->communicationService->sendDocument($request->user(), $company, $document, $payload);
