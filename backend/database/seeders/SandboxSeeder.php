...
class SandboxSeeder extends Seeder
{
    ...
    private function buildSaudiAddress(string $city, string $street, string $district, string $buildingNumber, string $postalCode, string $secondaryNumber): array
    {
        return [
    ...
    public function run(): void
    {
        ...
        $this->seedInventoryFoundation($actors['user'], $items);
    ...
    private function seedCompanyDirectory(Company $company): array
    {
        ...
        $expenseAccount = Account::query()->where('company_id', $company->id)->where('code', '6900')->firstOrFail();

        $customers = collect([
            ['display_name' => 'Red Sea Projects', 'legal_name' => 'Red Sea Projects Contracting Co.', 'tax_number' => '311245678900003', 'email' => 'finance@redseaprojects.sa', 'phone' => '+966550101201', 'address' => $this->buildSaudiAddress('Jeddah', 'Prince Sultan Road', 'Al Rawdah', '8101', '23432', '4321')],
            ['display_name' => 'Noor Medical Group', 'legal_name' => 'Noor Medical Group LLC', 'tax_number' => '311245678900013', 'email' => 'ap@noormedical.sa', 'phone' => '+966550101202', 'address' => $this->buildSaudiAddress('Riyadh', 'King Fahd Road', 'Al Olaya', '8202', '12214', '5432')],
            ['display_name' => 'Eastern Logistics Hub', 'legal_name' => 'Eastern Logistics Hub Ltd.', 'tax_number' => '311245678900023', 'email' => 'accounts@easternlogistics.sa', 'phone' => '+966550101203', 'address' => $this->buildSaudiAddress('Dammam', 'King Saud Road', 'Al Faisaliyah', '8303', '32271', '6543')],
        ...
        $suppliers = collect([
            ['display_name' => 'Najd Office Supply', 'legal_name' => 'Najd Office Supply Est.', 'tax_number' => '312345678900003', 'email' => 'orders@najdoffice.sa', 'phone' => '+966550202301', 'address' => $this->buildSaudiAddress('Riyadh', 'Imam Saud Bin Faisal Road', 'Al Sahafah', '8404', '13321', '7654')],
            ['display_name' => 'Gulf Facility Services', 'legal_name' => 'Gulf Facility Services Co.', 'tax_number' => '312345678900013', 'email' => 'billing@gfs.sa', 'phone' => '+966550202302', 'address' => $this->buildSaudiAddress('Khobar', 'Prince Turki Street', 'Al Ulaya', '8505', '34447', '8765')],
            ['display_name' => 'Makkah Travel Network', 'legal_name' => 'Makkah Travel Network LLC', 'tax_number' => '312345678900023', 'email' => 'finance@mtn.sa', 'phone' => '+966550202303', 'address' => $this->buildSaudiAddress('Jeddah', 'Hira Street', 'Al Zahraa', '8606', '23522', '9876')],
        ])->map(fn (array $payload) => Contact::query()->updateOrCreate(
            ['company_id' => $company->id, 'email' => $payload['email']],
            [
                'uuid' => Contact::query()
    ...
    private function seedSalesDocuments(User $user, $customers, $items, $costCenters): void
    {
        ...
        foreach ([
            ['title' => 'Retail Support Debit Note', 'source' => $invoices[0], 'issue_date' => '2026-04-03', 'quantity' => '0.25', 'notes' => 'Late delivery surcharge.'],
            ['title' => 'POS Deployment Debit Note', 'source' => $invoices[1], 'issue_date' => '2026-04-09', 'quantity' => '0.10', 'notes' => 'Additional installation fee.'],
            ['title' => 'Printer Refresh Debit Note', 'source' => $invoices[2], 'issue_date' => '2026-04-11', 'quantity' => '0.50', 'notes' => 'Extended warranty charge.'],
        ] as $debitDefinition) {
            $this->seedSalesDebitNote($salesService, $user, $debitDefinition);
        }
    }

    private function seedInventoryFoundation(User $user, $items): void
    {
        $inventoryService = app(InventoryService::class);

        foreach ($items->where('type', 'product')->values() as $index => $item) {
            ...
            $inventoryService->createReceipt($this->sandboxCompany, $user, [
                'item_id' => $item->id,
                'product_name' => $item->name,
                'inventory_type' => 'finished_good',
                'source' => 'purchase',
                'code' => sprintf('SBX-STK-%03d', $index + 1),
                'quantity_on_hand' => 25,
                'unit_cost' => $item->default_purchase_price,
                'transaction_date' => now()->subDays(45 - $index)->toDateString(),
                'reference' => sprintf('SBX-STOCK-%03d', $index + 1),
                'document_links' => [],
            ]);
        }
    }
    ...
    {
        $expenseAccount = Account::query()->where('company_id', $this->sandboxCompany->id)->where('code', '6900')->firstOrFail();
    ...
    private function seedPlatformCustomerCompanies(User $agentUser, array $plans): void
    {
        ...
        $companies = [
            ...
            ['owner_name' => 'Lina Al Zahrani', 'owner_email' => 'blue.dunes.owner@gulfhisab.sa', 'legal_name' => 'Blue Dunes Foods Co.', 'trade_name' => 'Blue Dunes Foods', 'tax_number' => '312998877600013', 'registration_number' => '4033344556', 'city' => 'Jeddah', 'plan' => $plans['trial'], 'status' => 'trialing', 'commission_amount' => 64, 'commission_status' => 'pending'],
            ['owner_name' => 'Majed Al Ghamdi', 'owner_email' => 'najd.facility.owner@gulfhisab.sa', 'legal_name' => 'Najd Facility Services Ltd.', 'trade_name' => 'Najd Facility', 'tax_number' => '312998877600023', 'registration_number' => '2055566778', 'city' => 'Dammam', 'plan' => $plans['free'], 'status' => 'active', 'commission_amount' => 28, 'commission_status' => 'earned'],
        ];
    ...
    private function seedSalesDebitNote(SalesDocumentService $service, User $user, array $definition): void
    {
        if (Document::query()->where('company_id', $this->sandboxCompany->id)->where('type', 'debit_note')->where('title', $definition['title'])->exists()) {
            return;
        }
        ...
        $service->issueDebitNote($this->sandboxCompany, $source, $user, [
            'issue_date' => $definition['issue_date'],
            'notes' => $definition['notes'],
            'lines' => [[
                'source_line_id' => $sourceLine->id,
