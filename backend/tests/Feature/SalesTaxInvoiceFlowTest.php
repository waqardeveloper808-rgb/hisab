...
class SalesTaxInvoiceFlowTest extends TestCase
{
    ...
    public function test_customer_invoice_payment_and_ledger_flow_works_end_to_end(): void
    {
        ...
        $finalizeResponse->assertOk()
            ...
            ->assertJsonPath('data.compliance_metadata.vat_ledger.direction', 'output')
            ->assertJsonPath('data.compliance_metadata.vat_ledger.line_count', 1);

        $transactionIntelligence = $this->postJson("/api/companies/{$companyId}/intelligence/transaction", [
            'contact_id' => $contactId,
        ...
        $transactionIntelligence->assertOk()
            ->assertJsonPath('data.suggestions.0.label', 'Post invoice to VAT and receivables automatically')
            ->assertJsonPath('data.reminders.0.label', 'Unpaid invoice follow-up');
        ...
        $reportIntelligence = $this->getJson("/api/companies/{$companyId}/intelligence/reports");
        $reportIntelligence->assertOk()
            ->assertJsonStructure([
                'data' => [
                    'suggestions',
                    'anomalies',
    ...
    public function test_multi_invoice_allocation_with_overpayment_creates_customer_credit(): void
    {
        ...
        $this->assertSame(4, $latestPaymentEntry->lines()->count());
    ...
    public function test_customer_advance_can_be_applied_to_a_new_invoice(): void
    {
        ...
        $secondInvoiceBeforeAdvance = Document::findOrFail($secondInvoice['document_id']);
        $this->assertSame('finalized', $secondInvoiceBeforeAdvance->status);
        $this->assertSame('115.00', $secondInvoiceBeforeAdvance->balance_due);
    ...
    public function test_invoice_credit_note_then_customer_refund_is_fully_traceable(): void
    {
        ...
        $filteredAuditTrail = $this->getJson("/api/companies/{$context['company_id']}/reports/audit-trail?from={$auditDate}&to={$auditDate}&event=document.credit_note_issued");
        $filteredAuditTrail->assertOk();
        $filteredRows = collect($filteredAuditTrail->json('data'));
        $this->assertCount(1, $filteredRows);
        $this->assertSame('document.credit_note_issued', $filteredRows->first()['event']);
    }

    public function test_debit_note_increases_receivable_and_updates_vat_reports(): void
    {
        $user = User::factory()->create();
        $this->actingAs($user);

        $context = $this->createCompanyContext('Debit Note Co');
        $invoice = $this->createFinalizedInvoice(
            $context['company_id'],
            $context['contact_id'],
            $context['item_id'],
        ...
        $debitResponse = $this->postJson("/api/companies/{$context['company_id']}/sales-documents/{$invoice['document_id']}/debit-notes", [
            'issue_date' => '2026-04-16',
            'status_reason' => 'Under-billed implementation fee',
            'lines' => [[
                'source_line_id' => $invoice['line_id'],
                'quantity' => 1,
                'unit_price' => 50,
                'description' => 'Additional implementation fee',
            ]],
        ]);

        $debitResponse->assertCreated()
            ->assertJsonPath('data.type', 'debit_note')
            ->assertJsonPath('data.grand_total', '57.50')
            ->assertJsonPath('data.source_document_id', $invoice['document_id']);

        $sourceInvoice = Document::findOrFail($invoice['document_id']);
        $debitNoteId = $debitResponse->json('data.id');
        $debitNote = Document::findOrFail($debitNoteId);
        $this->assertSame('finalized', $debitNote->status);
        $this->assertSame('57.50', $debitNote->balance_due);

        $receivablesAging = $this->getJson("/api/companies/{$context['company_id']}/reports/receivables-aging");
        $receivablesAging->assertOk();

        $agingNumbers = collect($receivablesAging->json('data'))->pluck('document_number');
        $this->assertTrue($agingNumbers->contains($sourceInvoice->document_number));
        $this->assertTrue($agingNumbers->contains($debitNote->document_number));
        ...
        $vatRow = collect($vatSummary->json('data'))->firstWhere('code', 'VAT15');
        $this->assertNotNull($vatRow);
        $this->assertSame('150.00', number_format((float) $vatRow['taxable_amount'], 2, '.', ''));
        $this->assertSame('22.50', number_format((float) $vatRow['tax_amount'], 2, '.', ''));

        $auditTrail = $this->getJson("/api/companies/{$context['company_id']}/reports/audit-trail");
        $auditTrail->assertOk();
        $events = collect($auditTrail->json('data'))->pluck('event');
        $this->assertTrue($events->contains('document.debit_note_issued'));
    }
