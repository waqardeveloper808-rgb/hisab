  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid gap-6 md:grid-cols-2 xl:grid-cols-4">
        <Card className="rounded-xl bg-white/95 p-5"><p className="text-sm font-semibold text-muted">Total users</p><p className="mt-3 text-3xl font-semibold text-ink">{snapshot.users.length}</p></Card>
        <Card className="rounded-xl bg-white/95 p-5"><p className="text-sm font-semibold text-muted">Active accountants</p><p className="mt-3 text-3xl font-semibold text-ink">{activeAccountants}</p></Card>
        <Card className="rounded-xl bg-white/95 p-5"><p className="text-sm font-semibold text-muted">Seat limit</p><p className="mt-3 text-3xl font-semibold text-ink">{snapshot.seatLimit ?? "Unlimited"}</p></Card>
        <Card className="rounded-xl bg-white/95 p-5"><p className="text-sm font-semibold text-muted">Remaining seats</p><p className="mt-3 text-3xl font-semibold text-ink">{snapshot.seatLimit === null ? "Unlimited" : Math.max(snapshot.seatLimit - activeAccountants, 0)}</p></Card>
      </div>
      ...
      {isPreview ? <div className="rounded-lg border border-sky-200 bg-sky-50 px-4 py-3 text-sm text-sky-800">Preview mode shows the user-management layout only. Sign in with a company admin account to manage admins, accountant seats, and permissions.</div> : null}
      {! canManageUsers ? <div className="rounded-lg border border-amber-200 bg-amber-50 px-4 py-3 text-sm text-amber-800">This account can view company users but cannot create, disable, or edit them.</div> : null}
