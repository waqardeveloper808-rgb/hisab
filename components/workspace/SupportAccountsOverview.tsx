...
export function SupportAccountsOverview() {
  ...
  return (
    <div className="space-y-6">
      ...
      {isPreview ? <div className="rounded-[1.2rem] border border-sky-200 bg-sky-50 px-4 py-3 text-sm text-sky-800">Preview mode shows the support-account layout only. Sign in as a super-admin to create or change platform operator accounts.</div> : null}
      {! canManageSupportAccounts ? <div className="rounded-[1.2rem] border border-amber-200 bg-amber-50 px-4 py-3 text-sm text-amber-800">This account can view support accounts but cannot create, disable, or edit them.</div> : null}
