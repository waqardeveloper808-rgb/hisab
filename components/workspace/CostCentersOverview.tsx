      ...
      <div className="grid gap-6 xl:grid-cols-[22rem_minmax(0,1fr)]">
        <Card className="rounded-xl bg-white/95 p-6">
          ...
          <div className="mt-5 space-y-3">
            {loading ? <div className="rounded-lg border border-dashed border-line bg-surface-soft px-4 py-6 text-sm text-muted">Loading cost centers...</div> : null}
            {!loading && !costCenters.length ? <div className="rounded-lg border border-dashed border-line bg-surface-soft px-4 py-6 text-sm text-muted">Create the first cost center to start tagging purchase documents.</div> : null}
            {costCenters.map((costCenter) => (
              <button key={costCenter.id} type="button" onClick={() => setDraft(costCenter)} className={["block w-full rounded-lg border px-4 py-3 text-left text-sm", draft.id === costCenter.id ? "border-primary bg-primary-soft" : "border-line bg-surface-soft hover:bg-white"].join(" ")}>
        ...
        <Card className="rounded-xl bg-white/95 p-6">
          ...
          <div className="mt-5 grid gap-4 md:grid-cols-2">
            ...
            <div className="md:col-span-2">
              ...
              <textarea id="cost-center-description" value={draft.description} onChange={(event) => setDraft((current) => ({ ...current, description: event.target.value }))} rows={4} className="block w-full rounded-lg border border-line-strong bg-white px-4 py-3.5 text-sm text-ink outline-none focus:border-primary/40 focus:ring-4 focus:ring-primary/10" />
            </div>
