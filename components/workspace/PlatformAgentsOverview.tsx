      ...
      <div className="grid gap-6 xl:grid-cols-[1.15fr_0.85fr]">
        ...
        <Card className="rounded-[1.8rem] bg-white/92 p-6">
          ...
          {selectedAgent.id ? (
            ...
            <div className="mt-5 rounded-[1.4rem] border border-dashed border-line bg-surface-soft px-4 py-6 text-sm text-muted">{isPreview ? "Sign in with a platform admin account to inspect live agents and commissions." : "Choose an agent from the table to edit it."}</div>
          )}
        </Card>
      </div>

      {isPreview ? <div className="rounded-[1.2rem] border border-sky-200 bg-sky-50 px-4 py-3 text-sm text-sky-800">Preview mode shows the agent-admin layout only. Sign in to search agents, adjust commission rates, and change activation status.</div> : null}
      {! canManageAgents ? <div className="rounded-[1.2rem] border border-amber-200 bg-amber-50 px-4 py-3 text-sm text-amber-800">This account can inspect agents but cannot change commission or activation status.</div> : null}
