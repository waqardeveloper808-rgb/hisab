        ...
        <Card className="rounded-[1.8rem] bg-white/92 p-6">
          ...
          {selectedCustomer.id ? (
            <div className="mt-5 space-y-5">
              ...
              {selectedPlan ? (
                <div className="rounded-[1.4rem] border border-line bg-surface-soft px-4 py-4 text-sm text-muted">
            ...
            <div className="mt-5 rounded-[1.4rem] border border-dashed border-line bg-surface-soft px-4 py-6 text-sm text-muted">{isPreview ? "Sign in with an admin or support account to inspect live customer companies." : "Choose a company from the table to inspect it."}</div>
          )}
        </Card>
      </div>

      {isPreview ? <div className="rounded-[1.2rem] border border-sky-200 bg-sky-50 px-4 py-3 text-sm text-sky-800">Preview mode shows the customer-control layout only. Sign in to search companies, edit subscription state, and save account changes.</div> : null}
      {! canManageCustomers ? <div className="rounded-[1.2rem] border border-amber-200 bg-amber-50 px-4 py-3 text-sm text-amber-800">This account can view customer companies but cannot change company status or profile fields.</div> : null}
