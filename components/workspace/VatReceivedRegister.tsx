...
import { listVatReceivedDetails, type VatReceivedDetailRecord } from "@/lib/workspace-api";
...
export function printFullVatRegister(rows: VatReceivedDetailRecord[]) {
  ...
  const rowsHtml = rows
    ...
    <tr>
      ...
      <td>${escapeHtml(row.invoiceNumber ?? row.documentNumber ?? "")}</td>
  ...
  printWindow.document.write(`<!doctype html>
    <html>
      <head>
        <title>VAT Received Register</title>
        <style>
      ...
      <body>
        <h1>VAT Received Register</h1>
        <p>Total rows: ${rows.length}</p>
        <table>
          <thead>
            <tr>
              <th>Type</th><th>Document #</th><th>Status</th><th>Issue date</th><th>Customer</th><th>Taxable</th><th>VAT</th><th>Source</th>
            </tr>
          </thead>
...
function exportVatReceivedCsv(rows: VatReceivedDetailRecord[]) {
  const head = ["Type", "Document #", "Status", "Issue date", "Customer", "Taxable amount", "VAT amount", "Source"];
  const lines = [head.join(","), ...rows.map((row) =>
    [
      row.documentType ?? "",
      row.invoiceNumber ?? row.documentNumber ?? "",
      row.status ?? "",
      row.issueDate ?? row.date ?? "",
      `"${String(row.customer ?? "").replaceAll('"', '""')}"`,
  ...
  a.href = url;
  a.download = `vat-received-register-${ymd}.csv`;
  a.click();
...
export function VatReceivedRegister() {
  const [rawRows, setRawRows] = useState<VatReceivedDetailRecord[]>([]);
  ...
  const [sortKey, setSortKey] = useState<
    "issueDate" | "documentNumber" | "customer" | "taxableAmount" | "vatAmount" | "type" | "status"
  >("issueDate");
  ...
  const load = useCallback(async () => {
    ...
    setLoadError(null);
    try {
      const rows = await listVatReceivedDetails({
        fromDate: fromDate || undefined,
        toDate: toDate || undefined,
      });
      setRawRows(rows);
    } catch (e: unknown) {
      setLoadError(e instanceof Error ? e.message : "Failed to load VAT received details.");
      setRawRows([]);
    } finally {
      setLoading(false);
    }
  ...
  const filtered = useMemo(() => {
    ...
    next = [...next].sort((a, b) => {
      switch (sortKey) {
        case "documentNumber": {
          const av = (a.documentNumber ?? a.invoiceNumber ?? "").toLowerCase();
          const bv = (b.documentNumber ?? b.invoiceNumber ?? "").toLowerCase();
  ...
  return (
    <div className="space-y-3">
      <div>
        ...
        <h1 className="text-lg font-semibold tracking-tight text-ink">VAT Received Register</h1>
        <p className="mt-1 text-xs text-muted">Output VAT from posted sales documents.</p>
      </div>
      ...
      <Card className="rounded-xl border border-line bg-white/95 p-3">
        <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-5">
          ...
          <div className="rounded-lg border border-line bg-surface-soft/60 p-2">
            <p className="text-[10px] font-semibold uppercase tracking-[0.1em] text-muted">VAT received total</p>
      ...
      <Card className="rounded-xl bg-white/95 p-3">
        <div className="flex flex-wrap items-end gap-2">
          <div className="min-w-[10rem] flex-1">
            <label className="mb-1 block text-xs font-semibold text-ink" htmlFor="vat-received-search">
              Search
            </label>
            <input
              id="vat-received-search"
              type="search"
              value={search}
              onChange={(e) => setSearch(e.target.value)}
              placeholder="Document, customer, type, status…"
              className="w-full rounded-md border border-line bg-white px-2.5 py-1.5 text-xs text-ink outline-none focus:border-primary"
            />
          </div>
          <div>
            <label className="mb-1 block text-xs font-semibold text-ink" htmlFor="vat-received-from">
              From
            </label>
            <input
              id="vat-received-from"
              type="date"
              value={fromDate}
          ...
          <div>
            <label className="mb-1 block text-xs font-semibold text-ink" htmlFor="vat-received-to">
              To
            </label>
            <input
              id="vat-received-to"
          ...
          <div>
            <label className="mb-1 block text-xs font-semibold text-ink" htmlFor="vat-received-type">
              Type
            </label>
            <select
              id="vat-received-type"
              value={typeFilter}
          ...
          <div>
            <label className="mb-1 block text-xs font-semibold text-ink" htmlFor="vat-received-status">
              Status
            </label>
            <select
              id="vat-received-status"
              value={statusFilter}
          ...
          <Button type="button" variant="secondary" size="xs" onClick={() => printFullVatRegister(filtered)}>
            Print full register
          </Button>
          <Button type="button" size="xs" onClick={() => exportVatReceivedCsv(filtered)}>
            Export CSV
          </Button>
          ...
          <table className="min-w-full text-xs">
            <thead className="border-b border-line bg-surface-soft/70">
              <tr>
                ...
                <th className="px-2 py-2 text-left">
                  <button type="button" className="font-semibold uppercase tracking-[0.08em] text-muted hover:text-ink" onClick={() => toggleSort("documentNumber")}>
                    Document #
                  </button>
                </th>
                <th className="px-2 py-2 text-left">
                ...
                <th className="px-2 py-2 text-left">
                  <button type="button" className="font-semibold uppercase tracking-[0.08em] text-muted hover:text-ink" onClick={() => toggleSort("customer")}>
                    Customer
                  </button>
                </th>
            ...
            <tbody>
              {loading ? (
                <tr>
                  <td colSpan={9} className="px-2 py-4 text-muted">
                    Loading VAT received register…
                  </td>
                </tr>
              ) : filtered.length === 0 ? (
                <tr>
                ...
                filtered.map((row) => {
                  ...
                  return (
                    <tr key={`${docId}-${row.invoiceNumber}`} className="border-t border-line/70">
                      <td className="px-2 py-1.5 text-ink">{displayType(row.documentType)}</td>
                      <td className="px-2 py-1.5 font-medium text-ink">{row.documentNumber ?? row.invoiceNumber}</td>
                      ...
                      <td className="px-2 py-1.5 text-ink">{row.customer}</td>
                      ...
                      <td className="px-2 py-1.5">
                        <Link
                          ...
                          data-testid={`vat-received-row-action-${docId}`}
                        >
                          View source
                        </Link>
                      </td>
