...
// Workspace V2 — schema-driven document renderer (Wafeq Format 2).
//
...
import type { DocumentRecord, Customer } from "@/lib/workspace-v2/types";
...
import {
  applyBoundaryDragPx,
  buildMinWidthGetter,
  fitRawWidthsToTarget,
  getItemsTableInnerTargetPx,
  widthsArrayToRecord,
} from "@/lib/workspace-v2/item-column-resize";
import {
  buildDocumentLayout,
  bilingualLabel,
  type LayoutInfoRow,
  ...
  return (
    ...
    >
      ...
      >
        {showEn ? (
          <>
            <div className="wsv2-wf-seller-name" style={{ color: textColors.english }}>
        ...
        {showAr ? (
          <>
            {layout.seller.nameAr || layout.seller.nameEn ? (
              <div className="wsv2-wf-seller-name" style={{ color: textColors.arabic }}>
                {layout.seller.nameAr || layout.seller.nameEn}
              </div>
            ) : null}
            {layout.seller.addressAr ? (
              <div className="wsv2-wf-line" style={{ color: textColors.arabic }}>
                {layout.seller.addressAr}
              </div>
            ) : null}
            {layout.seller.email ? (
    ...
    <div className="wsv2-wf-title">
      ...
      {language !== "english" ? (
        <div
          ...
          dir="rtl"
          lang="ar"
          style={{ color: ar, fontFamily: arFont }}
        >
          {layout.title.ar}
        </div>
      ) : null}
...
function ItemsSection({
  ...
  const emitWidths = useCallback(
    (arr: number[]) => {
      ...
      const r = fitRawWidthsToTarget(keys, arr, cap, minGetter);
            ...
            {language !== "arabic" ? (
              <tr className="wsv2-wf-hdr-en">
                {cols.map((col) => (
                  <th
                    key={col.key}
                    style={{ textAlign: col.align, color: textColors.english }}
            ...
            {language !== "english" ? (
              <tr className="wsv2-wf-hdr-ar">
                {cols.map((col) => (
                  <th
                    key={`${col.key}-ar`}
                    style={{ textAlign: col.align, color: textColors.arabic }}
              ...
              : language === "arabic"
                ...
                : (
                  <>
                    ...
                    <span className="wsv2-wf-totals-desc-ar wsv2-wf-type-ar" dir="rtl" lang="ar" style={{ color: textColors.arabic }}>
                      {row.labelAr}
                    </span>
                  </>
                )}
    ...
    >
      ...
      {qb.showCaptions ? (
        ...
        >
          ...
          {language !== "english" ? (
            <div className="wsv2-wf-qr-caption-ar wsv2-wf-type-ar" dir="rtl" lang="ar" style={{ color: textColors.arabic }}>
              {layout.qr.captionAr}
            </div>
          ) : null}
        </div>
    ...
    <div className="wsv2-wf-footer-bar">
      <span className="wsv2-wf-footer-text">
        {language === "english" ? (
          ...
          <>
            <span style={{ color: enC }}>{sellerEn}</span>
            <span style={{ color: arC }}> · {sellerAr}</span>
          </>
        ) : (
          <span style={{ color: enC }}>{sellerEn}</span>
        )}
...
export const WorkspaceV2DocumentRenderer = forwardRef<HTMLDivElement, RendererOptions>(
  function WorkspaceV2DocumentRenderer(props, ref) {
    const {
      schema,
      doc,
      seller,
