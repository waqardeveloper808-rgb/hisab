...
// Workspace V2 — Template Studio (schema-driven).
//
...
// shared <WorkspaceV2DocumentRenderer/> so that what you see here matches
...
import { buildInvoicePdf } from "@/lib/workspace-v2/exports/pdf";
import { buildInvoiceUbl } from "@/lib/workspace-v2/exports/xml";
import { downloadBytes, downloadXml } from "@/lib/workspace-v2/exports/download";
import {
  COLUMN_LABELS,
...
import { buildDocumentLayout } from "@/lib/workspace-v2/document-template-renderer";
import {
  buildMinWidthGetter,
...
export function WorkspaceV2TemplateStudio({ templateId }: Props) {
  ...
  const setItemColumnWidthPx = useCallback(
    (key: ColumnKey, nextPx: number) => {
      ...
      const nextRaw = [...raw];
      nextRaw[idx] = nextPx;
      const fitted = fitRawWidthsToTarget(
        keys,
        nextRaw,
        tableInnerTarget,
        columnMinGetter,
                ...
                {visible && plCol ? (
                  <div className="wsv2-col-width" aria-label="Column width in pixels">
                    ...
                    <input
                      ...
                      onChange={(e) => {
                        const n = parseInt(e.target.value, 10);
                        if (Number.isFinite(n) && n > 0) setItemColumnWidthPx(col, n);
                      }}
